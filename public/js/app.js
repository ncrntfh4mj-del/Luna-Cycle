const S = {
  lastPeriod:null, cycleLen:28, periodLen:5,
  currentPage:'dashboard', todayPhase:null,
  calYear:new Date().getFullYear(), calMonth:new Date().getMonth(),
  moodToday:null, suppChecked:{},
};
...
function saveSettings(){
  localStorage.setItem('luna_settings',JSON.stringify({lastPeriod:S.lastPeriod,cycleLen:S.cycleLen,periodLen:S.periodLen}));
}
...
async function fetchDashboard(){
  ...
  try{
    const res=await fetch('/api/cycle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lastPeriod:S.lastPeriod,cycleLen:S.cycleLen,periodLen:S.periodLen})});
    const data=await res.json();
    S.todayPhase=data.today.phase;
    renderHeader(data.today);
    renderWeekStrip(data.weekDays);
    renderPhaseCard(data.today);
    renderDday(data.upcoming);
    renderYogaPreview(data.today.phase);
    renderDietPreview(data.today.phase);
    renderSuppChecks(data.today.phase);
    renderMeditatePreview(data.today.phase);
    renderSidebarPhase(data.today);
    renderYogaPage(data.today.phase);
    renderNutritionPage(data.today.phase);
    renderJournalMeta(data.today);
...
}ㅛfunction renderYogaPage(phase){
  const w=WELLNESS[phase];if(!w)return;
  $('yogaSectionTitle').textContent=`${w.emoji} ${w.name} 아사나`;
  $('asanaGrid').innerHTML=w.yoga.map(a=>`
    <div class="asana-card">
      <div class="asana-emoji">${a.emoji}</div>
      <div class="asana-name">${a.name}</div>
      <div class="asana-sanskrit">${a.sanskrit}</div>
      <div class="asana-benefit">${a.benefit}</div>
      <div class="asana-meta"><span>⏱ ${a.duration}</span></div>
      <div class="asana-tip">💡 ${a.tip}</div>
    </div>`).join('');
  $('meditationCard').innerHTML=`
    <div class="med-card-title">🧘 ${w.meditation.title}</div>
    <div class="med-card-duration">⏱ ${w.meditation.duration}</div>
    <div class="med-card-guide">"${w.meditation.guide}"</div>
    <div class="med-card-meta">
      <div class="med-meta-item">🎵 <span>${w.meditation.music}</span></div>
      <div class="med-meta-item">🌬 <span>${w.meditation.breathwork}</span></div>
    </div>`;
  $('videoList').innerHTML=w.workout.videos.map(v=>`
    <a class="video-card" href="${v.url}" target="_blank" rel="noopener">
      <div class="video-thumb">▶</div>
      <div><div class="v-title">${v.title}</div><div class="v-meta"><span class="v-channel">${v.channel}</span></div></div>
      <div class="video-right"><span class="v-duration">⏱ ${v.duration}</span><span class="v-tag">${v.tag}</span></div>
    </a>`).join('');
}
...
function renderNutritionPage(phase){
  ...
  $('mealGrid').innerHTML=w.diet.meals.map(m=>`
    <div class="meal-card">
      <div class="meal-time-badge"><span class="meal-time-icon">${m.icon}</span><span>${m.time}</span></div>
      <div class="meal-name">${m.menu}</div>
      <div class="meal-reason">${m.reason}</div>
    </div>`).join('');
  $('avoidBox').innerHTML=`<strong>⚠ 오늘은 피해요</strong> ${w.diet.avoid}`;
  $('suppFullList').innerHTML=w.supplements.map(s=>`
    <div class="supp-full-item">
      <span class="sf-emoji">${s.emoji}</span>
      <div><div class="sf-name">${s.name}</div><div class="sf-reason">${s.reason}</div></div>
      <div class="sf-right"><div class="sf-dose">${s.dose}</div><div class="sf-timing">${s.timing}</div></div>
    </div>`).join('');
}
...
function bindJournal(){
  ...
  document.querySelectorAll('.chip').forEach(chip=>{
    chip.addEventListener('click',function(){
      if(this.dataset.sym==='없음')document.querySelectorAll('.chip').forEach(c=>c.classList.remove('active'));
      this.classList.toggle('active');
    });
  });
  $('journalSave').addEventListener('click',()=>{
    const text=$('journalQuick').value.trim();
    if(!text)return;
    saveJournalEntry(text);showToast('저널이 저장됐어요 ✓');
  });
  $('journalSaveFull').addEventListener('click',()=>{
    ...
    saveJournalEntry(text,symptoms,energy);
    showToast('저널이 저장됐어요 ✓');
    renderHistoryList();
  });
}
...
function renderHistoryList(){
  ...
  el.innerHTML=keys.slice(0,20).map(k=>{
    const j=journals[k];const[y,m,d]=k.split('-');
    return`<div class="history-item"><div class="hi-date">${y}년 ${parseInt(m)}월 ${parseInt(d)}일</div><div class="hi-mood">${j.mood||''}</div><div class="hi-text">${j.text?j.text.slice(0,80)+(j.text.length>80?'...':''):''}</div>${j.phase?`<div class="hi-phase">${PHASE_KO[j.phase]||j.phase}</div>`:''}</div>`;
  }).join('');
}
...
function highlightMood(emoji){
  document.querySelectorAll('.mood-btn').forEach(b=>b.classList.toggle('selected',b.dataset.mood===emoji));
}
...
function bindSettings(){
  $('settingsSave').addEventListener('click',()=>{
    ...
    saveSettings();fetchDashboard();renderCalendar();
    showToast('설정이 저장됐어요 ✓');navigateTo('dashboard');
  });
}

function bindCalNav(){
  $('calPrev').addEventListener('click',()=>{S.calMonth--;if(S.calMonth<0){S.calMonth=11;S.calYear--;}renderCalendar();});
  $('calNext').addEventListener('click',()=>{S.calMonth++;if(S.calMonth>11){S.calMonth=0;S.calYear++;}renderCalendar();});
}
...
async function renderCalendar(){
  ...
  try{
    const res=await fetch('/api/calendar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({lastPeriod:S.lastPeriod,cycleLen:S.cycleLen,periodLen:S.periodLen,year:S.calYear,month:S.calMonth})});
    const data=await res.json();
    renderCalDays(data.days);renderPhaseChips(data.phaseSummary);
  }catch(e){console.error(e);}
}
...
function renderCalDays(days){
  ...
  days.forEach(({day,phase,label,emoji})=>{
    ...
    el.className=cls;
    el.innerHTML=`<span class="cal-day-num">${day}</span><span class="cal-day-icon">${emoji}</span>`;
    el.title=label;grid.appendChild(el);
  });
}

function renderPhaseChips(summary){
  $('phaseSummaryRow').innerHTML=summary.map(p=>`
    <div class="phase-chip">
      <div class="pc-emoji">${p.emoji}</div>
      <div class="pc-name">${p.name}</div>
      <div class="pc-dates">${p.from}~${p.to}</div>
    </div>`).join('');
}
