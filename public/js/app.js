...
function refreshDashboard(){
  ...
  renderPhaseSummaryCards(data.phaseSummary);
}
...
function renderPhaseSummaryCards(phaseSummary){
  const el=$('phaseSummaryRow');if(!el)return;
  el.innerHTML=phaseSummary.map(p=>`<div class="phase-chip"><div class="pc-emoji">${p.emoji}</div><div class="pc-name">${p.name}</div><div class="pc-dates">${p.from}~${p.to}</div></div>`).join('');
}
